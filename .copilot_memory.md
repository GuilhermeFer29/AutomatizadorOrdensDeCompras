...
- [x] Implementado retry logic com exponential backoff em `app/core/database.py`
- [x] Adicionado health check para MySQL no `docker-compose.yml`
- [x] Configurado `depends_on` com `condition: service_healthy` para API e Worker
- [x] Adicionado `broker_connection_retry_on_startup=True` no Celery
- [x] Configurado comando `--host-cache-size=0` no MySQL para remover warnings
- [x] Health check adicionado para Redis/broker
- Pronto para validação: após `docker-compose down -v && docker-compose up --build`, o banco deve conectar sem erros
- **Bloqueio resolvido**: Implementadas correções para o erro `2003 (HY000): Can't connect to MySQL server on 'db:3306' (111)`

### Arquivos modificados nesta correção:
1. `app/core/database.py` - Retry logic implementado
2. `docker-compose.yml` - Health checks e dependências configuradas
3. `app/core/celery_app.py` - Configuração `broker_connection_retry_on_startup` adicionada
