...
from app.core.database_async import get_session_factory
from app.models.models import Produto, OfertaProduto, Fornecedor, OrdemDeCompra, PrecosHistoricos
    ...
    async def _query():
        ...
        async with TenantAwareSession() as session:
            ...
            ofertas_query = (
                ...
                .where(OfertaProduto.disponivel == True)
            )
            
            if tenant_id:
                ofertas_query = ofertas_query.where(OfertaProduto.tenant_id == tenant_id)
            
            ofertas_query = ofertas_query.order_by(OfertaProduto.preco.asc())
            ...
            for oferta, fornecedor in ofertas:
                ofertas_list.append({
                    ...
                    "quantidade_minima": oferta.quantidade_minima or 1,
        ...
        async with TenantAwareSession() as session:
            ...
            if fornecedor_id is None:
                ofertas_query = (
                    ...
                    .where(OfertaProduto.disponivel == True)
                    .order_by(OfertaProduto.preco.asc())
                    .limit(1)
                )
                result = await session.execute(ofertas_query)
                ...
                if melhor_oferta:
                    fornecedor_id = melhor_oferta.fornecedor_id
                    preco_unitario = melhor_oferta.preco
                else:
                    return json.dumps({
                ...
                oferta_query = (
                    select(OfertaProduto)
                    .where(OfertaProduto.produto_id == produto.id)
                    .where(OfertaProduto.fornecedor_id == fornecedor_id)
                    .where(OfertaProduto.tenant_id == tenant_id)
                )
                result = await session.execute(oferta_query)
                oferta = result.scalar_one_or_none()
                preco_unitario = oferta.preco if oferta else Decimal("0.00")
