...
from app.models.models import ModeloGlobal, Produto
...
def train_product_model_task(self, sku: str, optimize: bool = False, n_trials: int = 30) -> dict[str, Any]:
    ...
    """
    LOGGER.info("ml.tasks.train_product.start", sku=sku, optimize=optimize, n_trials=n_trials)
    ...
    try:
        ...
        LOGGER.warning("ml.tasks.train_product.insufficient_data", sku=sku, error=str(e))
        return {
            "status": "skipped",
            "sku": sku,
            "reason": str(e),
        ...
        LOGGER.error("ml.tasks.train_product.failed", sku=sku, error=str(e), exc_info=True)
        raise
...
def train_all_products_task(self, optimize: bool = False, limit: int = None) -> dict[str, Any]:
    ...
    for idx, produto in enumerate(produtos, 1):
        LOGGER.info("ml.tasks.train_all.progress", current=idx, total=total, sku=produto.sku)
        ...
        try:
            ...
            LOGGER.error("ml.tasks.train_all.product_failed", sku=produto.sku, error=str(e))
...
def retrain_global_model_task(self) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, produto in enumerate(produtos, 1):
            LOGGER.info("ml.tasks.retrain_global.progress", current=idx, total=total, sku=produto.sku)

            try:
                train_model_for_product(sku=produto.sku, optimize=False, n_trials=0, backtest=False)
                success_count += 1
            except InsufficientDataError:
                LOGGER.warning("ml.tasks.retrain_global.insufficient_data", sku=produto.sku)
            except Exception as e:
                LOGGER.error("ml.tasks.retrain_global.product_failed", sku=produto.sku, error=str(e))
